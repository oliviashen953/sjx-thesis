...
create_directories <- function(trait_name) {
  dir_create(paste0("2.2_RACE_", trait_name, "/training_files"))
  dir_create(paste0("2.2_RACE_", trait_name, "/testing_files"))
  dir_create(paste0("2.2_RACE_", trait_name, "/test_subgroup_files"))
}
...
perform_sampling <- function(data, trait_name, n_train = 3000, n_groups = 20, n_group_size = 3000, iterations = 37) {
  ...
  for (iter in 1:iterations) {
    ...
    if(nrow(eligible_afr_individuals) >= n_train) {
      ...
      cat("Sampled 3000 individuals with race 'afr' for supervised training in", trait_name, "\n")
    } else {
      train_data <- eligible_afr_individuals
      cat("Not enough individuals with race 'afr' available in", trait_name, ". Only", nrow(train_data), "available.\n")
    }
    
    # Save training data to csv
    write.csv(train_data, paste0("2.2_RACE_", trait_name, "/training_files/", trait_name, "_train_data_iteration_", iter, ".csv"), row.names = FALSE)
    
    # Get the rest as testing data
    test_data <- data[!rownames(data) %in% rownames(train_data), ]
    write.csv(test_data, paste0("2.2_RACE_", trait_name, "/testing_files/", trait_name, "_test_data_iteration_", iter, ".csv"), row.names = FALSE)
    ...
    for (group in 1:n_groups) {
      ...
      write.csv(group_data, paste0("2.2_RACE_", trait_name, "/test_subgroup_files/", trait_name, "_subgroup_", group, "_iteration_", iter, ".csv"), row.names = FALSE)
    }
  }
}
...
height_train_data_iteration_37 <- read.csv("2.2_RACE_height/training_files/height_train_data_iteration_37.csv")
height_test_data_iteration_37 <- read.csv("2.2_RACE_height/testing_files/height_test_data_iteration_37.csv")
height_subgroup_1_iteration_37 <- read.csv("2.2_RACE_height/test_subgroup_files/height_subgroup_1_iteration_37.csv")

bmi_train_data_iteration_37 <- read.csv("2.2_RACE_bmi/training_files/bmi_train_data_iteration_37.csv")
bmi_test_data_iteration_37 <- read.csv("2.2_RACE_bmi/testing_files/bmi_test_data_iteration_37.csv")
bmi_subgroup_1_iteration_37 <- read.csv("2.2_RACE_bmi/test_subgroup_files/bmi_subgroup_1_iteration_37.csv")

brc_train_data_iteration_37 <- read.csv("2.2_RACE_brc/training_files/brc_train_data_iteration_37.csv")
brc_test_data_iteration_37 <- read.csv("2.2_RACE_brc/testing_files/brc_test_data_iteration_37.csv")
brc_subgroup_1_iteration_37 <- read.csv("2.2_RACE_brc/test_subgroup_files/brc_subgroup_1_iteration_37.csv")
