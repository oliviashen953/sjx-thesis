...
perform_ols_regression <- function(dataset_path, dataset_name, iterations) {
  for (iter in 1:iterations) {
    # Load training data
    train_data <- read.csv(paste0(dataset_path, "/RACE_", dataset_name, "_train_data_iteration_", iter, ".csv"))
    ...
    saveRDS(coefficients, paste0("RACE_coefficients_", dataset_name, "_iteration_", iter, ".rds"))
  }
}
...
perform_lasso_regression <- function(dataset_path, dataset_name, iterations, lambda_seq, n_folds = 10) {
  for (iter in 1:iterations) {
    ...
    # Save coefficients to an RDS file
    saveRDS(coefficients, paste0("RACE_lasso_coefficients_", dataset_name, "_iteration_", iter, ".rds"))
  }
}
...
perform_best_r_squared <- function(dataset_path, dataset_name, iterations) {
  for (iter in 1:iterations) {
    ...
    saveRDS(best_column, paste0("RACE_best_pgs_column_", dataset_name, "_iteration_", iter, ".rds"))
  }
}

# Example usage with your datasets for OLS
perform_ols_regression("2.2_RACE_height/training_files", "height", iterations = 37)
...
old_dir <- "~/"
ols_new_dir <- "2.1_RACE_ols_supervised_learning_coefficients"
lasso_new_dir <- "2.1_RACE_lasso_supervised_learning_coefficients"
best_pgs_new_dir <- "2.1_RACE_best_pgs_columns"
...
ols_files <- list.files(path = old_dir, pattern = "^RACE_coefficients_.*\\.rds$", full.names = TRUE)
for (file in ols_files) {
  # Extract the base name
  base_name <- basename(file)
  
  # Construct new name
  new_base_name <- gsub("coefficients_", "RACE_ols_coefficients_", base_name)
...
lasso_files <- list.files(path = old_dir, pattern = "^RACE_lasso_coefficients_.*\\.rds$", full.names = TRUE)
...
best_pgs_files <- list.files(path = old_dir, pattern = "^RACE_best_pgs_column_.*\\.rds$", full.names = TRUE)
...
#### RENAME FILES UNDER 2,1_RACE_ols_supervised_learning_coefficients ####

# Load necessary library
library(fs)

# Function to rename files to remove duplicate "RACE_RACE" prefix
correct_filenames <- function(directory) {
  # List all files in the directory
  files <- list.files(path = directory, pattern = "RACE_RACE", full.names = TRUE)
  ...
  for (file in files) {
    ...
    # Construct new name by removing the duplicate "RACE_RACE" prefix
    new_base_name <- gsub("RACE_RACE", "RACE", base_name)
...
correct_filenames("2.1_RACE_ols_supervised_learning_coefficients")
