...
input_output_folders <- list(
  list(input = "2.2_height_ranked_transposed_files", output = "2.4_height_TIE_ranked_transposed_files"),
  list(input = "2.2_bmi_ranked_transposed_files", output = "2.4_bmi_TIE_ranked_transposed_files"),
  list(input = "2.2_brc_ranked_transposed_files", output = "2.4_brc_TIE_ranked_transposed_files")
)

# Define file patterns
patterns <- c("ranked_transposed_height_subgroup_\\d+_iteration_\\d+\\.csv",
...
for (i in seq_along(input_output_folders)) {
  process_files_in_folder(input_output_folders[[i]]$input, input_output_folders[[i]]$output, patterns[i])
}
