...
library(readr)
best_pgs_columns_bmi <- read_csv("NORMAL/bmi_best_pgs/best_pgs_columns_bmi.csv")
View(best_pgs_columns_bmi)
...
occurrences <- table(best_pgs_columns_bmi$Best_PGS_Column)
...
library(readr)
NORMAL_bmi_summary_standardized_difference_across_groups_and_iterations <- read_csv("NORMAL_bmi_summary_standardized_difference_across_groups_and_iterations.csv")
View(NORMAL_bmi_summary_standardized_difference_across_groups_and_iterations)

# Load the data
BMI_summary <- NORMAL_bmi_summary_standardized_difference_across_groups_and_iterations

# Define the specific methods you're interested in
ra_methods <- c("mc4_rank","mct_rank", "bcs_rank", "logs_rank","sbs_rank", "avg_ra_rank")
pgs_methods <- c("lasso_rank", "ols_rank", "PGS001943_rank", "avg_pgs_rank")
...
pgs_labels <- setNames(c(
  ...
  "Supervised3000 Best PGS Model: PGS001943",
  "Average PGS Rank"
), all_methods)
...
method_colors <- c(
  ...
  "Supervised3000 Best PGS Model: PGS001943" = "#999999", # Grey
...
method_shapes <- c(
  ...
  "Supervised3000 Best PGS Model: PGS001943" = 4,  # X
...
plot <- ggplot(filtered_methods, aes(x = group_percentile, y = mean_standardized_difference, group = rank_method,
  ...
  labs(y = "Mean Standardized Difference", x = "Group Percentile",
       title = "BMI (Supervised Random 3000): Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = unname(method_shapes))))

# Save or display the plot
plot
ggsave("NORMAL_BMI_OG_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)
